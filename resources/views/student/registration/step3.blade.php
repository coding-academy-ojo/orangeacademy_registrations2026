...
@section('title', 'Step 3 - Academy & Cohort')
        ...
        <div class="card-body p-4 pt-0">
            <form method="POST" action="{{ route('student.save.enrollment') }}">
                ...
                <div class="row g-4 mb-4">
                    @foreach($academies as $academy)
                        @if($academy->cohorts->count())
                            @php
                                // Map links based on user request
                                $mapLink = '#';
                                $img = 'https://images.unsplash.com/photo-1541339907198-e08756dedf3f?auto=format&fit=crop&q=80&w=400';

                                if (stripos($academy->name, 'amman') !== false || stripos($academy->name, 'عمان') !== false) {
                            ...
                            <div class="col-md-6 col-lg-3">
                                <label class="d-block w-100">
                                    <input type="radio" name="academy_selection" class="d-none peer-academy-radio"
                                    ...
                                    @foreach($academy->cohorts as $cohort)
                                        <div class="col-md-6">
                                            <label class="d-block w-100 h-100">
                                                <input type="radio" name="cohort_id" value="{{ $cohort->id }}" class="d-none peer-radio"
                ...
                <div class="d-flex justify-content-between mt-5 pt-3 border-top">
                    <a href="{{ route('student.step', 2) }}" class="btn btn-outline-secondary px-4 py-2 rounded-3"><i
